...
/**
 * <code>QueryLoader</code> is a registry for sets of queries so that multiple
...
public class QueryLoader
{
	...
	/**
	 * Maps query set names to Maps of their queries. 使用缓存的形式进行加载的
	 */
	 ...
	 * cached Map. The properties file to load can be in either line-oriented or
	 * XML format. XML formatted properties files must use a <code>.xml</code>
	 * file extension.
	 *
	 * @param path
	 ...
	 *             if the XML properties file is invalid
	 ...
	 * a properties file at the given path. The properties file can be in either
	...
	protected Map<String, String> loadQueries(String path) throws IOException
	{
		...
		@SuppressWarnings(
		{ "rawtypes", "unchecked" }) // load() always creates <String,String>
										// entries
